...
if is_raspberrypi():
    try:
        from rpi_ws2812 import PixelStrip, Color
        ...
        # Compatibility wrapper class for PixelStrip
        class WS2811Wrapper:
            def __init__(self, led_count, led_pin, led_freq, led_dma, led_invert, led_brightness, led_channel, strip_type):
                self.strip = PixelStrip(led_count, led_pin, led_freq, led_dma, led_invert, led_brightness, led_channel, strip_type)
                self.led_count = led_count
                self.strip_type = strip_type
                self.has_white = (strip_type & 0x18000000) != 0
            
            def begin(self):
                return self.strip.begin()
            
            def show(self):
                return self.strip.show()
            
            def setPixelColor(self, index, color):
                return self.strip.setPixelColor(index, color)
            ...
            def cleanup(self):
                if hasattr(self.strip, 'cleanup'):
                    return self.strip.cleanup()
        ...
        def ws2811_channel_t_gpionum_set(channel, gpio):
            """Set GPIO pin (no-op, set during initialization)"""
            pass
        ...
        def ws2811_t_freq_set(strip, freq):
            """Set frequency (no-op, set during initialization)"""
            pass
        
        def ws2811_t_dmanum_set(strip, dma):
            """Set DMA channel (no-op, set during initialization)"""
            pass
        ...
        print(f"Warning: Could not import rpi_ws2812: {e}")
